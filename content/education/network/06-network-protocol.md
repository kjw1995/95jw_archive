...
{{< callout type="info" >}}
**용어 정리**
- **프로토콜 (Protocol)**: 서로 다른 시스템이 통신하기 위한 공통 규칙·언어. 서로 다른 제조사의 장비도 TCP/IP라는 공통 언어로 통신한다
- **프로토콜 3요소**: 구문(형식)·의미(내용)·타이밍(순서/속도)
- **RFC**: IETF가 발행하는 인터넷 표준 문서
- **TCP**: 연결 지향·신뢰성 있는 전송 프로토콜 ([24. TCP의 구조](../24-tcp-structure) 참고)
- **UDP**: 비연결·저지연 전송 프로토콜 ([27. UDP의 구조](../27-udp-structure) 참고)
- **프로토콜 스택**: 계층별 프로토콜을 쌓아 구성한 집합 ([07. OSI/TCP-IP](../07-osi-tcpip-model) 참고)
{{< /callout >}}
