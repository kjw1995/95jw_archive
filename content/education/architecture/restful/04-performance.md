...
API 성능은 사용자 경험과 직결된다. 캐싱, 비동기 처리, 부분 업데이트 같은 기법으로 응답 속도를 높이고 서버 부하를 줄인다.
...
| 전략 | 핵심 효과 |
|:---|:---|
| Caching | 응답 지연·서버 부하·대역폭 감소 |
| Async | 긴 작업을 논블로킹으로 처리 |
| Partial Update | PATCH로 필요한 필드만 전송 |
| Pagination | 대량 데이터를 분할 제공 |
| Compression | gzip/br로 전송량 축소 |
| Rate Limiting | 과부하 방지, 공정한 사용 보장 |

## 2. 캐싱의 원리

캐싱은 요청에 대한 응답을 임시 저장해 동일 요청에 저장된 응답을 반환하는 기술이다.
...
| 캐싱 권장 | 캐싱 비권장 |
|:---|:---|
| 정적 리소스(이미지, JS, CSS) | 실시간 데이터 |
| 자주 변경되지 않는 데이터 | 사용자별 개인정보 |
| CPU 집약적 연산 결과 | 트랜잭션 결과 |
| 공용 참조 데이터 | 인증 필요 리소스 |
...
{{< callout type="info" >}}
두 헤더가 모두 있으면 `Cache-Control`이 우선한다. HTTP/1.1 환경에서는 `Cache-Control` 사용을 권장한다.
{{< /callout >}}
...
| private | 브라우저만 캐시 가능 |
| no-cache | 매번 서버에 재검증 (저장은 함) |
| no-store | 캐시 저장 자체 금지 |
...
| If-Match | 요청 | 조건부 PUT/DELETE - 일치 시 수행 |
...
| replace | 값 교체 | `{"op":"replace", "path":"/name", "value":"New"}` |
...
| 분야 | 이점 |
|:---|:---|
| SPA (단일 페이지 앱) | 대역폭 절약, 빠른 UI 반응 |
| 실시간 협업 시스템 | Google Docs 스타일 동시 편집, 충돌 해결 |
| 오프라인 데이터 동기화 | 변경사항만 전송하여 동기화 효율화 |
| 대용량 문서 편집 | 전체 대신 변경된 부분만 전송 |

## 9. Rate Limiting

API 과부하를 방지하고 공정한 사용을 보장하는 기법이다.
...
| DO | DON'T |
|:---|:---|
| 정적 리소스에 장기 캐시 + 버전 URL | PUT/POST 요청 결과 캐싱 |
| Cache-Control과 ETag 조합 사용 | 쿼리 파라미터 요청 캐싱 |
| CDN 레이어 활용 | 개인 데이터를 public 캐시 |
| GET 요청에만 캐싱 적용 | Expires와 max-age 동시 사용 |
| 공용은 public, 개인은 private | ETag와 Last-Modified 동시 사용 |

{{< callout type="warning" >}}
인증이 필요한 개인 데이터를 `public`으로 캐시하면 다른 사용자에게 노출될 수 있다. 개인 응답에는 반드시 `private` 또는 `no-store`를 명시한다.
{{< /callout >}}
...
- **ETag**: 리소스 버전 지문, 조건부 요청에 사용
- **Conditional Request**: `If-*` 헤더로 변경 여부를 판별하는 요청
- **202 Accepted**: 요청 수락, 처리는 비동기로 진행
- **SSE**: 서버가 클라이언트에 단방향 이벤트 스트림을 푸시
- **JSON Patch**: 연산 기반 부분 업데이트(RFC 6902)
- **JSON Merge Patch**: 병합 기반 부분 업데이트(RFC 7396)
- **Rate Limiting**: 시간당 요청 수를 제한하여 남용·과부하 방지
- **CDN**: 엣지 노드에 공용 캐시를 두는 콘텐츠 전송 네트워크
{{< /callout >}}
