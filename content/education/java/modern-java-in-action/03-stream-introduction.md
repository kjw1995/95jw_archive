...
```
컬렉션: 특정 시점에 모든 것이 존재하는 공간 (DVD)
스트림: 시간적으로 흩어진 값의 집합 (스트리밍)
```
...
**스트림은 단 한 번만 소비**할 수 있다. 다시 탐색하려면 데이터 소스에서 새로운 스트림을 생성해야 한다. 컬렉션과 달리 스트림은 데이터를 저장하지 않고 흘려보내기 때문이다.
{{< /callout >}}
...
| **실행 시점** | 최종 연산 호출 전까지 미실행 | 호출 시 전체 파이프라인 실행 |
...
| **루프 퓨전** | filter와 map이 한 번의 순회로 병합되어 처리 |

{{< callout type="info" >}}
위 출력에서 주목할 점: 요소 하나가 `filter → map`을 **연달아** 통과한 뒤 다음 요소로 넘어간다(루프 퓨전). 또한 `limit(3)` 덕분에 9개 요리 전부가 아니라 3개를 채우는 순간 처리가 멈춘다(쇼트서킷). 게으른 평가가 만들어내는 자동 최적화다.
{{< /callout >}}
...
| **게으른 계산** | 최종 연산 호출 전까지 중간 연산 미실행 |

{{< callout type="info" >}}
**핵심 포인트:**
- 스트림은 **선언형**으로 데이터를 처리하는 파이프라인이다
- **중간 연산**은 게으르게 실행되고, **최종 연산**이 파이프라인을 실행한다
- 게으른 평가는 **루프 퓨전·쇼트서킷** 같은 자동 최적화를 가능하게 한다
- 내부 반복으로 **병렬화**와 **최적화**를 자동으로 처리한다
- 스트림은 **단 한 번만 소비** 가능하다
{{< /callout >}}
