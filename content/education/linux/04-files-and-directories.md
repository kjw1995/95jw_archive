...
{{< callout type="info" >}}
실무에서는 대부분 **심볼릭 링크**를 사용한다. 디렉터리에도 걸 수 있고 다른 파일시스템(디스크)도 가리킬 수 있기 때문이다. 단, 원본을 삭제하면 링크가 깨져(dangling) `ls`에서 빨간색으로 표시되고 접근 시 오류가 난다. 하드 링크는 같은 데이터를 가리키는 또 하나의 이름이라 원본을 지워도 데이터가 남는다.
{{< /callout >}}
