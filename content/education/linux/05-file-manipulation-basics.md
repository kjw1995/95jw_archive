...
`rm -rf`는 확인 없이 디렉터리를 통째로 지우며, 휴지통이 없는 리눅스에서는 복구가 거의 불가능하다. 특히 변수를 쓴 `rm -rf "$DIR/"`에서 `$DIR`가 비어 있으면 `rm -rf /`가 되어 시스템을 파괴한다. 실행 전 변수 값을 `echo`로 확인하고, 와일드카드를 쓸 때는 먼저 `ls`로 대상을 점검하는 습관을 들이자.
{{< /callout >}}
