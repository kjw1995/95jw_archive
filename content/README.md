...
### (주)티엔씨테크놀로지 (2025.11 ~)
- **크림페이 선불·PG 결제 플랫폼**: 원천사 API 연동, 충전·결제·취소 연계, 정산·세금계산서·현금영수증 발행 배치 구축
- **Kovan 선불전자지급 시스템**: 선불 API 연동규격서(OpenAPI) 작성·배포, 회원·계좌·OTC·크레딧 API 개발
- **채비 거래 중계 모듈**: 이기종 결제 모듈 간 거래 중계 계층 설계 및 결제수단 발급·취소 단일 규격화
- **금융결제원 영중소가맹점 정산**: 관리·조회 API 및 연동 실패 건 재처리(Retry) Job 설계
- **현대캐피탈 전표 회계연동 배치**: Spring Batch·Quartz 기반 전표 변환·전송 배치 구축
- **사내 공통 모듈 버전업**: Java·Spring 버전업 및 버전업 표준 절차 문서화
